...
class TaskProvider extends ChangeNotifier {
  ...
  Future<bool> changeTaskStatus(String id, String newStatus) async {
    final response = await ApiCaller.getRequest(
      URL: Urls.ChangeStatusURL(id, newStatus),
    );
    ...
    if (response.isSuccess) {
      ...
      await Future.wait([fetchTaskByStatus(newStatus), fetchTaskCounts()]);
  ...
  Future<bool> deleteTask(String id) async {
    ...
    if (response.isSuccess) {
      ...
      await Future.wait([fetchTaskCounts()]);
  ...
  Future<bool> addTask(String title, String description) async {
    ...
    if (response.isSuccess) {
      await Future.wait([fetchTaskByStatus('New'), fetchTaskCounts()]);
      return true;
    } else {
      errorMessage = response.responseData['data'];
      notifyListeners();
